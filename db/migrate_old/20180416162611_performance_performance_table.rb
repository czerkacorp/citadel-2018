class PerformancePerformanceTable < ActiveRecord::Migration[5.1]
  def up
    query = <<-SQL
      CREATE TABLE performance_performance (
    ...
    seeds = File.read("db/sql_seeds/performance_performance_seeds.sql")
  ...
  def down
    query = <<-SQL
      DROP TABLE performance_performance;
    SQL
    execute query
  end
end
